...
def home(request,pk):
    if request.user.is_patient:
            ...
            bot = Doctor_Information.objects.get(doctor_id=11)
            print("bot",bot)
...
def send_chat(request):
    ...
    if request.method == 'POST':
        ...
        if post['user_to'] == "28":
            ...
            # Call the customChatGPT method
            response = chatbot.customChatGPT(post['message'], [])
            u_from = User.objects.get(id=post['user_to'])
            u_to = User.objects.get(id=post['user_from'])
            insert = chatMessages(user_from=u_from, user_to=u_to, message=response)
            try:
                insert.save()
                resp['status'] = 'success'
            except Exception as ex:
